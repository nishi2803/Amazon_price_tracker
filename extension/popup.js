window.onload = async () => {
    ...
    let func4 = () => {
        document.querySelectorAll(".remove").forEach(el => {
            el.addEventListener("click", (e) => {
                ...
                chrome.runtime.sendMessage({type : "remove_item", item_id : el.dataset.id}, (response)=>{
                    if(response.removed){
                        starting();
                    }
                    else{
    ...
    let get_user_data_from_server = async (data) => {
        ...
        let da = await fetch(`http://localhost:${port}/getdata`, {
            method: "POST",
        ...
        if (item_data.products.length) {
            let selected_item_html = "<ul style = 'list-style : none;'>"
            item_data.products.forEach(item => {
                selected_item_html += `<br><br><li class = "list"><a href = "${item.productUrl}" target = "_blank" class = "linking">${item.productName}</a><a class = "remove" data-id = ${item._id}>Remove</a><br><ul><li class = "price">current price : ${item.currentPrice} INR</li><br><li class= "threshprice">threshold Price : ${item.thresholdPrice} INR</li></ul></li><hr class = "break">`;
            })
